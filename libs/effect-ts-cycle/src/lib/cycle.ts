...
export interface Cycle<R, State, A extends Action, O extends A> {
    _A: A
    _O: O
    _R: R
    _S: State
...
type CyclesEnvType<CS extends AnyCycle> = UnionToIntersection<
    Env<Exclude<CS, Cycle<unknown, any, any, any>>>
>;
...
export function embed<CS extends A.NonEmptyArray<AnyCycle>>(
    ...cycles: CS
        ...
        provider: (
            _: T.Effect<CyclesEnvType<typeof cycles[number]>, never, unknown>
        ) => T.Effect<any, never, unknown>
    ...
    type REnv = CyclesEnvType<CSType>;
