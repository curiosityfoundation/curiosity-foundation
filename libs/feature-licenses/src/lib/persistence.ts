...
import { accessUUIDM } from '@curiosity-foundation/feature-uuid';
...
const makeLicensePersistence = () => ({
  insertUnclaimedLicense: ({ deviceId }: DeviceId) =>
    pipe(
      T.do,
      T.tap(() =>
        verbose('encoding unclaimed license')
      ),
      T.bind(
        'doc',
        () => accessUUIDM(({ generate }) =>
          pipe(
            generate,
            T.chain((_id) =>
              encodeUnclaimedLicense({
      ...
      T.tap(() =>
        verbose('inserting unclaimed license')
      ),
      T.bind('inserted', ({ doc }) =>
      ...
      T.tap(() =>
        verbose('unclaimed license inserted')
      ),
    ),
